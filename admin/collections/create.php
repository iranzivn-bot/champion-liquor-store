...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name !== '') {
        $check = $pdo->prepare('SELECT COUNT(*) FROM collections WHERE name = :name');
    ...
    if ($slug !== '') {
        $check = $pdo->prepare('SELECT COUNT(*) FROM collections WHERE slug = :slug');
    ...
    if (count($errors) === 0) {
        $stmt = $pdo->prepare('
            INSERT INTO collections (name, slug, description, image, category_id, sort_order, status)
            VALUES (:name, :slug, :description, :image, :category_id, :sort_order, :status)
        ');
        $stmt->execute([
            ':name'        => $name,
            ':slug'        => $slug,
        ...
        logActivity((int) $_SESSION['user_id'], $_SESSION['user_name'] ?? '', $_SESSION['user_role'] ?? '', 'collections', 'created', $slug, 'Created collection: ' . $name);

        setFlashMessage('success', 'Collection "' . htmlspecialchars($name) . '" created successfully.');
...
<div class="admin-content-wrapper">
    <main class="admin-content">
        ...
        <div class="admin-card">
            ...
            <p class="text-muted">Fill in the fields below to add a new collection.</p>
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="d-flex gap-2">
                    <button type="submit" class="btn" style="background: var(--admin-primary); color: #fff;">Save Collection</button>
