...
$code = trim($_GET['code'] ?? '');
if ($code === '') {
    setFlashMessage('error', 'Invalid product code.');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($barcode !== '') {
        $check = $pdo->prepare('SELECT COUNT(*) FROM products WHERE barcode = :barcode AND id != :id');
        $check->execute([':barcode' => $barcode, ':id' => $id]);
        if ($check->fetchColumn() > 0) {
            $errors[] = 'Another product with this barcode already exists.';
        }
    }
    ...
    if ($name !== '') {
        ...
        if ($check->fetchColumn() > 0) {
            $errors[] = 'Another product with this name already exists.';
        }
    }
    ...
    if ($slug !== '') {
        ...
        if ($check->fetchColumn() > 0) {
            $errors[] = 'Another product with this slug already exists.';
        }
    }
    ...
    if (count($errors) === 0) {
        $stmt = $pdo->prepare('
            UPDATE products
               SET category_id = :category_id, brand_id = :brand_id, name = :name, slug = :slug,
                   description = :description, price = :price, discount_price = :discount_price,
                   stock_quantity = :stock_quantity, image = :image, barcode = :barcode, status = :status
             WHERE id = :id
        ');
        $stmt->execute([
        ...
        logActivity((int) $_SESSION['user_id'], $_SESSION['user_name'] ?? '', $_SESSION['user_role'] ?? '', 'products', 'updated', $productCode, 'Updated product: ' . $name . ' (' . $productCode . ')');

        setFlashMessage('success', 'Product "' . htmlspecialchars($name) . ' (' . $productCode . ')" updated successfully.');
        redirect(SITE_URL . 'admin/products/index.php');
    }
    }
}
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control bg-light"
                           value="<?= htmlspecialchars($productCode) ?>" readonly>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="description" name="description" rows="4"
                              maxlength="5000"><?= htmlspecialchars($description) ?></textarea>
                </div>
                ...
                <div class="row">
                    <div class="col-md-4 mb-3">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" class="form-control" id="price" name="price"
                                   value="<?= htmlspecialchars((string) $price) ?>" required min="0" step="0.01">
                        </div>
                    </div>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" class="form-control" id="discount_price" name="discount_price"
                                   value="<?= htmlspecialchars((string) $discount_price) ?>" min="0" step="0.01">
                        </div>
                    </div>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="number" class="form-control" id="stock_quantity" name="stock_quantity"
                               value="<?= (int) $stock_quantity ?>" min="0">
                    </div>
                </div>
                ...
                <?php if ($existingImage): ?>
                    <div class="mb-3">
                        ...
                        <div>
                            <img src="<?= SITE_URL ?>uploads/products/<?= htmlspecialchars($existingImage) ?>"
                ...
                <div class="mb-3">
                    ...
                    <div class="input-group">
                        ...
                        <button type="button" class="btn btn-outline-secondary" id="generateBarcodeBtn"
                                title="Generate barcode from product name">
                            <i class="bi bi-upc-scan"></i> Generate
                        </button>
                    </div>
                    <div class="form-text">Optional. Must be unique if provided. Leave empty to remove.</div>
                </div>
                ...
                <div class="d-flex gap-2">
                    <button type="submit" class="btn" style="background: var(--admin-primary); color: #fff;">Update Product</button>
